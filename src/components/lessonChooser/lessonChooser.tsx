...
const LessonChooser: React.FC<{
  ...
  const lessonsData = Object.keys(lessons).map((key) => {
    ...
    return lesson?.lesson ? (
      <Lesson
        lessonClicked={() => lessonChoseHandler(Number(key))}
  ...
  const Overlay: React.FC<{ overlayClicked: PropsFn }> = (prop) => {
    return (
      <div
        onClick={prop.overlayClicked}
        className={`${styles.overlay} ${
          props.showLessons ? styles.overlay__hide : ""
        } `}
      >
        {" "}
      </div>
  ...
  return (
    <div className={`${styles.stages} `}>
      {ReactDom.createPortal(
        <Overlay overlayClicked={props.showLessonsClicked} />,
        document.getElementById("overlay-root") as HTMLElement,
      )}
      <div className={styles.book}>
        {props.showLessons ? (
          <RiBook2Fill
            onClick={props.showLessonsClicked}
            size="2em"
            className={styles.stages_icon}
          />
        ) : (
          <RiBookOpenFill
            onClick={props.showLessonsClicked}
      ...
      <div
        className={`${styles.stages__modal} ${
          !props.showLessons ? styles.stages__modal__show : ""
        }`}
      >
        <h3 className={styles.modal__title}>Choose Lesson</h3>
        <div className={styles.lessons}>{lessonsData}</div>

        <button onClick={continueHandler} className={`${styles.button_21}`}>
          Continue
        </button>
      </div>
    </div>
