...
const MyInput: React.FC<{
  show?: boolean;
  hintIndex: number;
  resetHintIndex: PropsFn;
  answerWord: string;
  showAnswer: (arg: boolean) => void;
  ...
  useEffect(() => {
    if (
      ...
      audio &&
      insertedText.length &&
      insertedText.toLocaleLowerCase().trim() ===
        props.answerWord.toLowerCase().trim()
    ) {
      playAudio();
  ...
  function getLetterStyle(letterIndex: number): React.CSSProperties {
    let isMatching =
      insertedText.toLowerCase().charAt(letterIndex) ===
      props.answerWord.toLowerCase().charAt(letterIndex);
  ...
  return (
    <div>
      {props.answerWord.split("").map((letter, index) => (
        <span
          onClick={() => props.showAnswer(true)}
          className={`answer ${props.show ? "showAnswer" : ""}`}
          key={index}
